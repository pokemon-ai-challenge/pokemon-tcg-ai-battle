...
def choose_hidden_zone_action(obs: Observation) -> list[int]:
    """Choose cards to move into deck, deck bottom, or prize."""
...
def choose_hidden_zone_placement_action(
    ...
    """Choose TO_DECK / TO_DECK_BOTTOM / TO_PRIZE targets."""
    if obs.select is None:
    ...
    scored_views.sort(key=lambda item: (-item[0], item[1]))
...
def _forced_full_hand_return(obs: Observation, views: list[Any]) -> list[int] | None:
    if obs.select is None or obs.current is None:
        return None
    if obs.select.context not in {SelectContext.TO_DECK, SelectContext.TO_DECK_BOTTOM}:
        return None
    if not views:
        return None
...
def _score_deck_return_candidate(
    ...
    """Score hidden-zone choices with owner and source-area awareness."""
    if view.owner_is_self is True:
        if view.area == AreaType.HAND:
            return _score_self_hand_to_hidden_zone(obs, view, all_views, card_data_by_id)
        if view.area == AreaType.DISCARD:
            return _score_self_discard_to_hidden_zone(obs, view, all_views, card_data_by_id)
        if view.area in {AreaType.DECK, AreaType.LOOKING}:
            return _score_self_deck_or_looking_reorder(obs, view, all_views, card_data_by_id)
        if view.area in {AreaType.ACTIVE, AreaType.BENCH}:
            return _score_self_in_play_to_hidden_zone(obs, view, all_views, card_data_by_id)
        return -1.0

    if view.owner_is_self is False:
        if view.area == AreaType.HAND:
            return _score_opponent_hand_disruption(obs, view, all_views, card_data_by_id)
        if view.area == AreaType.DISCARD:
            return _score_opponent_discard_recovery(obs, view, all_views, card_data_by_id)
        return _score_opponent_return_candidate(obs, view, all_views, card_data_by_id)

    return -1.0


def _score_self_hand_to_hidden_zone(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
...
def _score_self_discard_to_hidden_zone(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
) -> float:
    recovery_score = _self_recovery_score(obs, view, all_views, card_data_by_id)

    if obs.select is None:
        return recovery_score
    if obs.select.context == SelectContext.TO_PRIZE:
        prize_ok_score = _self_prize_ok_score(obs, view, all_views, card_data_by_id)
        return prize_ok_score - recovery_score
    if obs.select.context == SelectContext.TO_DECK_BOTTOM:
        return recovery_score - _self_delayed_recovery_penalty(view, card_data_by_id)

    return recovery_score


def _score_self_deck_or_looking_reorder(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
...
def _score_self_in_play_to_hidden_zone(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
) -> float:
    keep_score = _self_keep_score(obs, view, all_views, card_data_by_id) + 3
...
def _score_opponent_hand_disruption(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
...
def _score_opponent_discard_recovery(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
) -> float:
    opponent_value = _score_opponent_return_candidate(obs, view, all_views, card_data_by_id)
...
def _self_keep_score(
    ...
    if card_data.cardType == CardType.POKEMON:
        ...
        score += 7
        if _ready_attacker_count(obs, owner_is_self=True, card_data_by_id=card_data_by_id) == 0:
            score += 2
        if energy_count > 2:
...
def _self_recovery_score(
    obs: Observation,
    view: Any,
    all_views: list[Any],
    card_data_by_id: dict[int, CardData],
) -> float:
    score = _self_keep_score(obs, view, all_views, card_data_by_id)
    card_data = card_data_by_id.get(view.card_id) if view.card_id is not None else None
    if card_data is None:
        return score
...
def _self_delayed_recovery_penalty(
    view: Any,
    card_data_by_id: dict[int, CardData],
) -> float:
    card_data = card_data_by_id.get(view.card_id) if view.card_id is not None else None
    ...
    if card_data.cardType in {CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY}:
        return 2.0
    if card_data.cardType == CardType.POKEMON and card_data.evolvesFrom:
        return 2.5
    if card_data.cardType == CardType.POKEMON and (card_data.ex or card_data.megaEx):
        return 1.5
    return 1.0
