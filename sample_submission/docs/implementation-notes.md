...
- 本番設定（10 秒/手）ではさらにロールアウト数が増え精度が上がる。

---

## フェーズ 5: 動的時間マネージャ（リソース最大活用）

### 背景

「時間に余裕があるので 1 手の探索時間を倍以上にしたい」という要望を受けた。
ただし **単純に固定値を倍にするのは危険**。

`diag_timing.py` で実測したところ:
- MC 探索が走る決定は対ランダムで **1 ゲーム 33 回**
- 長い接戦（自己対戦）では player あたり **70〜120 回**

固定 8 秒 × 120 回 = 960 秒で、10 分制限を簡単に超える。
MC 探索は MAIN 以外の複数候補選択（TO_HAND, SWITCH, DAMAGE_COUNTER 等）でも走るため、
呼び出し回数が想像以上に多い。

### 設計: 残り時間ベースの動的予算

ゲーム全体 600 秒を「使ってよい総計算時間」とみなし、
`GAME_TIME_CAP = 540 秒`（60 秒マージン）を上限に設定。

```python
def _decision_budget() -> float:
    remaining = GAME_TIME_CAP - _GAME_TIME_USED
    budget = remaining / RESERVE_DIV          # RESERVE_DIV = 40
    budget = clamp(budget, PER_MOVE_MIN=1.0, TIME_BUDGET_SEC=10.0)
    return min(budget, remaining)             # 残りを絶対超えない
```

`agent()` の `finally` 節で実消費時間を `_GAME_TIME_USED` に累積する
（成功・例外いずれの経路でも必ず計上）。

**数学的保証**: 各手の予算 ≤ remaining/40 < remaining なので、
累積使用時間は GAME_TIME_CAP に漸近するが**決して超えない**。

| ゲーム長 | 総使用時間 | 1 手目 | 最終手 |
|---------|-----------|--------|--------|
| 33 手 | 293 秒 | 10 秒 | 6.3 秒 |
| 70 手 | 443 秒 | 10 秒 | 2.5 秒 |
| 122 手 | 517 秒 | 10 秒 | 1.0 秒 |
| 200 手 | 540 秒 | 10 秒 | 0 秒 |

序盤は旧来の 3.3 倍（10 秒）を使い精度最大化、終盤は自動で節約。

### 新ゲーム検出

ローカルテスト（`battle_start` に直接デッキを渡す）では
`obs.select is None`（デッキ宣言）を経由しないため累積がリセットされない。
対策として **ターン数が巻き戻ったら新ゲームとみなしてリセット**する処理を追加。
