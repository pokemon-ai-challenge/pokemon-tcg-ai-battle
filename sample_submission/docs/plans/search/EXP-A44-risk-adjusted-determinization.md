...
5. 「ルール的に互角」な候補集合 T を作る（タイブレーク方式、下記）
6. |T| < 2 なら None（ルールが優劣を決めきれている＝介入しない）。
   |T| >= 2 なら T の中で score[k] が最大の候補を返す。
   ログ上は「探索が上書きした/しなかった」を記録する
```

#### タイブレーク方式（v2。初版のスコア混合は棄却済み）

```text
top = candidates[0].rule_score            # ルールスコア最大
T = { k : top - candidates[k].rule_score <= tie_margin(top) }
tie_margin(top) = max(tie_abs, tie_ratio * |top|)
```

既定は `tie_ratio = 0.05`（トップの5%以内）、`tie_abs = 1.0`。`T` の中だけリスク集約スコアで
並べ替え、`T` の外の候補には一切触らない。

**初版（スコア混合）を棄却した理由 — 実測に基づく**:

初版は `最終スコア = (1 - w) * min-max正規化ルールスコア + w * リスク集約スコア`（w=0.25）だった。
これは**構造的に上書きが起きない**。min-max正規化はトップ候補を必ず 1.0、最下位を必ず 0.0 にする一方、
このリポジトリのルールスコアは有界でない（`priorities/attack.py` の score は生ダメージ量で、
KO可能技は `_KO_BONUS=1000` が乗る。対して下準備は70程度）。スコアが 1000 対 70 なら2位以下は
正規化後ほぼ 0.0 に潰れ、寄与が最大 0.25 しかない勝率項では 0.75 の差を原理的に覆せない。

**実測（2026-07-25、20戦）: 501回発火して overrides は 1回（0.2%）**。この状態で200戦A/Bを回すと
Treatment と Control がほぼ同一プログラムになり、勝率 54.5%（z=1.27）・優勢局面の勝ち切り率 +9.0pt
という数字が出たが、**1試合あたり0.05手しか違わない比較なのでこれは全てノイズであり、リスク評価の
効果を示す証拠にはならない**（撤退トリガー再設計が「トリガーが稀すぎて『一切逃げない』と同一プログラムに
収束し50.0%」になったのと同じ構図）。

**教訓**: 有界でないルールスコアと [0,1] の確率を線形混合してはいけない。混合比の議論以前に、
正規化の形で介入余地が決まってしまう。ML側は「ルールが決めきれない時だけ働く」従属的な使い方
（RL prior が候補を絞るだけで最終決定はしない、等）に寄せる。

**重要（過去の教訓の反映）**: それでも介入は控えめから始める。`tie_ratio` を初版から 0.2 以上に
広げない（広げるほどルールベースの積み上げを壊す方向に働く）。
...
### 5.10 【必須】A/Bを回す前の上書き率チェック

**勝率A/Bを回す前に、必ず override 率を実測すること。**

- 判定: 20戦程度で `overrides / fired >= 0.05`（発火のうち5%以上で実際に手が変わる）
- これを下回る場合、Treatment と Control はほぼ同一プログラムであり、**勝率A/Bは何を測っても
  ノイズしか返さない**。A/Bを回さず、まず介入余地（tie_margin / ゲート条件）を見直す
- 初版がこのチェックを持たずA/B 200戦を回して 0.2% と判明した経緯は §5.2 を参照
