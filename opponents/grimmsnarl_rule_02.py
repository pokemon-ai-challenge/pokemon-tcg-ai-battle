"""ルールベース マリィのオーロンゲex 対戦相手(02: 非メノコ純ビート型, bono, LB 1150.8)。

出典: `kaggle_replays/meta_analysis/archetype_decks/marnie_grimmsnarl_ex/02.csv`
...
このデッキはユキメノコ不採用(`has_snow=False`)・ノコッチ/ノココッチ採用という
プロファイルの型違いで、`grimmsnarl_core.build_profile` がプランB(オーロンゲアグロ)
寄りの挙動に自動で分岐する(契約 §E)。

league から使うときは `--deck-b opponents/grimmsnarl_ex_deck_02.csv` を必ず対応させること
...
_DECK_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "grimmsnarl_ex_deck_02.csv")
