...
def agent(obs_dict: dict) -> list[int]:
    ...
    """
    def to_builtin_data(value):
        if value is None or isinstance(value, (str, int, float, bool)):
            return value
        if isinstance(value, list):
            return [to_builtin_data(v) for v in value]
        if isinstance(value, tuple):
            return [to_builtin_data(v) for v in value]
        if isinstance(value, dict):
            return {k: to_builtin_data(v) for k, v in value.items()}
        if hasattr(value, "items"):
            return {k: to_builtin_data(v) for k, v in value.items()}
        if hasattr(value, "__dict__"):
            return {k: to_builtin_data(v) for k, v in vars(value).items()}
        return value

    def choose_minimum_required(select) -> list[int]:
        # 必要最低限の数，候補の先頭から選ぶ
    ...
    def choose_action(obs: Observation) -> list[int]:
        ...
        validate_choice(select, chosen)
        return chosen

    obs: Observation = to_observation_class(to_builtin_data(obs_dict))
    if obs.select == None:
