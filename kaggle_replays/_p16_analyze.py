"""Phase16 §18-§27: L1/L2 集計 + delta 距離/完全一致/action type 別の後解析。

ラベルは Q0 vs DeltaQ_D1(データ側のキー名は Phase13 互換で "S3" のまま持つ)。
...
def main() -> None:
    ...
    ap.add_argument("--data", default="kaggle_replays/_p16_m*.jsonl.gz")
    ap.add_argument("--out", default="kaggle_replays/value_net/phase16_report.json")
    args = ap.parse_args()
    ...
    # ---- §21 delta 距離別(事前三分位で固定)----
    dd = sorted(g["delta_distance"] for g in G)
    q1, q2 = dd[len(dd) // 3], dd[2 * len(dd) // 3]
    rep["delta_distance_bounds"] = {"q33": round(q1, 6), "q66": round(q2, 6)}

    def dband(g):
        d = g["delta_distance"]
        return "small" if d <= q1 else "medium" if d <= q2 else "large"

    rep["by_delta_distance"] = subset_table(G, dband)

    # ---- §22 delta23 完全一致 ----
    ident = [g for g in G if g.get("delta_identical")]
    identl2 = [g for g in ident if g["l2_delta"] is not None]
    rep["delta_identical"] = {
        "groups": len(ident),
        "rate": round(len(ident) / len(G), 4),
        "mean_l1_delta": round(st.mean([g["l1_delta"] for g in ident]), 4) if ident else None,
        "mean_l2_delta": round(st.mean([g["l2_delta"] for g in identl2]), 4) if identl2 else None,
        "l2_decisive": sum(1 for g in identl2 if g["w_l2"] in ("Q0", "S3")),
        "mean_abs_l2_delta": round(st.mean([abs(g["l2_delta"]) for g in identl2]), 4)
                             if identl2 else None,
        "note": "delta23 が同一なのにモデル選択が違う group。Transformer T-B の材料。",
    }
    rep["delta_distance_stats"] = {
        "mean": round(st.mean([g["delta_distance"] for g in G]), 4),
        "median": round(st.median([g["delta_distance"] for g in G]), 4)}

    # ---- §27 raw entity distance(before の局面特性)----
    def ent_size(g):
        e = g.get("entity_before")
        if not e:
            return None
        s_ = e["self"]
        return len(s_.get("hand", [])) + len(s_["discard"])
    sizes = [ent_size(g) for g in G if ent_size(g) is not None]
    rep["entity_before_summary"] = {"n": len(sizes),
                                    "mean_hand_plus_discard": round(st.mean(sizes), 2)
                                    if sizes else None}
    ...
    ATYPE = {7: "play", 8: "attach", 9: "evolve", 10: "ability", 13: "attack"}
    rep["by_action_type_named"] = subset_table(
        G, lambda g: ATYPE.get(g["action_type_q0"], "other:%d" % g["action_type_q0"]))
    ...
    rep["examples"] = ex
    cases = {"A_same_delta_diff_outcome": [], "B_big_delta_delta_wins": [],
             "C_big_delta_no_diff": [], "D_identical_delta_big_l2": []}
    for g in sorted(l2G, key=lambda x: -abs(x["l2_delta"])):
        d = g["delta_distance"]
        big = d > q2
        rec = {"group_id": g["group_id"], "turn": g["turn"], "arch": g["arch"],
               "delta_distance": round(d, 4),
               "action_type_q0": g["action_type_q0"], "action_type_s3": g["action_type_s3"],
               "l1_delta": round(g["l1_delta"], 4), "l2_delta": round(g["l2_delta"], 4),
               "teacher_cls": (g.get("teacher") or {}).get("cls"),
               "hand_size": (len((g.get("entity_before") or {}).get("self", {}).get("hand", []))
                             if g.get("entity_before") else None)}
        if g.get("delta_identical") and abs(g["l2_delta"]) > 0.05:
            if len(cases["D_identical_delta_big_l2"]) < 10:
                cases["D_identical_delta_big_l2"].append(rec)
        elif d <= q1 and abs(g["l2_delta"]) > 0.05:
            if len(cases["A_same_delta_diff_outcome"]) < 10:
                cases["A_same_delta_diff_outcome"].append(rec)
        elif big and g["l2_delta"] > 0.05:
            if len(cases["B_big_delta_delta_wins"]) < 10:
                cases["B_big_delta_delta_wins"].append(rec)
        elif big and abs(g["l2_delta"]) <= 0.05:
            if len(cases["C_big_delta_no_diff"]) < 10:
                cases["C_big_delta_no_diff"].append(rec)
    rep["cases"] = cases
