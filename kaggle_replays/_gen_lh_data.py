"""Phase19: Long-Horizon Action-Q dataset。
...
def _continue_to_terminal(node, me, policy_model, max_steps):
    ...
    if last is not None:
        try:
            ...
            pass
    return {"outcome": outcome, "steps": steps, "reason": reason}
...
def _flush():
    out = _HERE / "_lh_{}.jsonl.gz".format(OPTS["tag"])
...
def _probe(obs, policy_model):
    ...
    is_audit = (_ctx["in_game"] + len(GROUPS)) % OPTS["audit_every"] == 0
    blk_b = None
    if is_audit:
    ...
    short = _short_teacher(obs, me, cand, policy_model)
    if short is None:
        STATS["short_fail"] += 1
    ...
    def agg(rs):
        oc = [x["outcome"] for x in rs]
        return {"outcomes": oc, "mean": round(statistics.mean(oc), 6),
...
def main() -> None:
    ...
    ap.add_argument("--offset", type=int, default=700000)
