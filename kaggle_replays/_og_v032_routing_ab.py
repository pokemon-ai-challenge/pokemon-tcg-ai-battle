"""og_v032(新デッキ)× crustle限定: ルーティング(crustle専用方策)のA/B。
...
COMMON = dict(
    ...
    deck_b_path=str(_ROOT / "kaggle_replays/meta_analysis/archetype_decks_g2/crustle/01.csv"),
    weights_a_path=str(_ROOT / "sample_submission/ptcg_ai/learning/policy_weights_ogerpon_teal_ex_rl_mixogerpon.json"),
    weights_b_path=str(_ROOT / "sample_submission/ptcg_ai/learning/policy_weights_crustle_g2.json"),
...
def main() -> None:
    out = {}
    for tag, cfg_a, seed in (("baseline", "abl_5_full", 960000),
                             ("routing08", "abl_5_full_og_routing08", 961000)):
