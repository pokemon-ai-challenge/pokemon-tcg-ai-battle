"""Phase19.11 Stage B+C: high-M terminal reference と checkpoint Value validity。

1本の terminal trajectory から C1b/C2/C4 の Frozen Value と最終 outcome を同時に取る。
これで (a) 高精度 ΔJ(Stage B)と (b) V_t vs 実 terminal の対応(Stage C)が同時に得られる。

強制するのは **最初の1手だけ**(§39)。TES が見つけた turn-plan 全体は固定しない。
...
def tes_scores(obs, me, cand, policy_model, m, base):
    """TES(M=2): 候補を強制 -> 自分の現ターン終了まで -> Frozen Value(§2)。"""
    out = []
    for i in cand:
        vals = []
        for k in range(m):
            r = _tes_rollout(obs, me, i, base + 40000 + k, policy_model)
            if r is not None:
                vals.append(r)
        if not vals:
            return None
...
def _tes_rollout(obs, me, action, seed, policy_model):
    ev = EVALS["value"]
    try:
    ...
    try:
        ...
        while steps < 300:
            ...
            if s is None:
                break
            if s.result != -1:
                return 1.0 if s.result == me else (0.0 if s.result == 1 - me else 0.5)
            actor = s.yourIndex
            if prev == me and actor != me:
                return float(ev.evaluate(s, me))
        ...
        return float(ev.evaluate(s, me)) if s is not None else None
    finally:
...
def _continue_ckpt(node, me, ev, max_steps, policy_model):
    """terminal まで進めつつ C1b/C2/C4 の Frozen Value を記録(Stage C)。"""
    ck = {}
    me_to_opp = opp_to_me = 0
    prev, steps, term = me, 0, None
    last = None
    ...
    out = None
    if term is not None:
        out = 1.0 if term == me else (0.0 if term == 1 - me else 0.5)
    elif last is not None:
        out = float(ev.evaluate(last, me))
    return {"outcome": out, "ckpt": ck, "steps": steps, "terminal": term}
...
def _one_branch(obs, me, action, seed, l1_eval, policy_model):
    ...
    try:
        try:
            ...
            return None
        if OPTS.get("ckpt_mode"):
            return _continue_ckpt(child, me, EVALS["value"], OPTS["max_steps"],
                                  policy_model)
...
def _probe(obs, policy_model):
    ...
    tes = tes_scores(obs, me, cand, policy_model, OPTS.get("tes_m", 2), dseed)
    if tes is None:
        STATS["tes_fail"] += 1
        return None
    qs = {"Q0": eq_scores(MODELS["EQ"], orows, cid, EX.extract(state, me), ent_afters),
          "S3": tes}
    ...
    def agg(rs):
        if OPTS.get("ckpt_mode"):
            ...
            per = [{"outcome": x["outcome"], "ckpt": x["ckpt"], "steps": x["steps"],
                    "terminal": x["terminal"]} for x in rs]
            return {"n": len(rs), "J": round(statistics.mean(oc), 6) if oc else None,
                    "trajs": per,
                    "terminal_rate": sum(1 for x in rs if x["terminal"] is not None) / len(rs)}
...
def main() -> None:
    ...
    ap.add_argument("--offset", type=int, default=2100000)
    ap.add_argument("--tes-m", type=int, default=2)
    ap.add_argument("--ckpt-mode", type=int, default=0)
    ...
    OPTS.update(n_cont=args.n_cont, max_steps=args.max_steps,
                null_control=args.null_control, tes_m=args.tes_m,
    ...
    EVALS["l1"] = leaf_eval_module.build_evaluator({"kind": "value"})
    EVALS["value"] = EVALS["l1"]   # 教師と別の凍結評価器
    _install()
    ...
    out = _HERE / "_p1911_{}.jsonl.gz".format(args.tag)
