"""Phase17: Transition-aware Transformer 用データ収集。

Phase15 に加えて **候補ごとの raw after-entity 全体**を保存する(§4/§6)。
Phase16 で「delta23 は同一だが raw entity は違うはず」を直接検証できなかった反省。
...
_ctx = {"game": -1, "recording": False, "me": 0, "in_game": 0, "opp": "", "first": True,
        "hist": deque(maxlen=HIST_N), "state_rows": [], "probe_rows": [],
        "pending": None}
QUOTA = None
MODELS: dict = {}
...
M_DETS = 4          # expected delta に使う determinization 数(§7 D2)


def _teacher(obs, me, cand, policy_model, n_block, deltas_out=None, seeds_out=None,
             ents_out=None):
            ...
            try:
                ...
                for i in cand:
                    try:
                        ...
                        continue
                    try:
                        # 教師が既に踏んでいる search_step をそのまま使う(§26 再利用)
                        if deltas_out is not None and len(deltas_out.get(i, [])) < M_DETS:
                            stt = ch.observation.current
                            deltas_out.setdefault(i, []).append(
                                EX.summarize(stt, me) if stt is not None else None)
                            if seeds_out is not None:
                                seeds_out.setdefault(i, []).append(det_seed)
                            # raw after-entity は最初の determinization のみ保存(§6)
                            if ents_out is not None and i not in ents_out and stt is not None:
                                ents_out[i] = EX.extract(stt, me)
...
def _probe_state(obs, policy_model):
    ...
    if STATS["states"] < OPTS["max_states"]:      # states ファイルだけを打ち切る
        STATES.append(row)
    ...
    blocks = _teacher(obs, me, cand, policy_model, NB, deltas_out=deltas,
                      seeds_out=dseeds, ents_out=ents)
    if blocks is None:
    ...
    pr = {
        ...
        "candidates": [{
            ...
            "afters": [[round(x, 4) for x in a] if a is not None else None
                       for a in deltas.get(cand[n], [])],
            "det_seeds": dseeds.get(cand[n], []),
            "after_entity": ents.get(cand[n]),
        } for n in range(len(cand))],
        "before": [round(x, 4) for x in EX.summarize(state, me)],
        "m_dets": M_DETS,
        "legal_option_types": [int(getattr(o.type, "value", o.type)) for o in select.option],
    ...
    PROBES.append(pr)
    _ctx["probe_rows"].append(pr)
    if len(PROBES) % 40 == 0:
        _flush(OPTS.get("tag", "x"))
    _ctx["pending"] = pr
...
def _flush(tag):
    """途中経過を書き出す(全滅防止)。最終書き出しと同じ形式。"""
    for name, rows in (("states", STATES), ("probes", PROBES)):
        out = _HERE / "_p17_{}_{}.jsonl.gz".format(name, tag)
...
def _install():
    ...
    def sa(obs, config=None):
        ...
        # 実際に選ばれた選択肢を、直前に作った group へ記録(§14 selected_option)
        if _ctx["pending"] is not None:
            _ctx["pending"]["selected_option"] = int(out[0]) if out else None
            _ctx["pending"] = None
...
def main() -> None:
    ...
    for gi in range(args.max_games):
        if QUOTA.total >= args.probe_target:
            break
        ...
        _ctx.update(game=g, recording=True, me=0 if p0 else 1, in_game=0, opp=arch,
                    first=p0, state_rows=[], probe_rows=[], pending=None)
        _ctx["hist"].clear()
        ...
        for r in _ctx["state_rows"] + _ctx["probe_rows"]:
            r["game_outcome"] = won
        if gi % 5 == 0:
    ...
    for name, rows in (("states", STATES), ("probes", PROBES)):
        out = _HERE / "_p17_{}_{}.jsonl.gz".format(name, args.tag)
