"""Phase13: Q0-expanded vs S3 の独立長期評価(L1 / L2)。
...
GAME_OFFSET = 50000            # 20000/30000/40000 は既使用
GROUPS: list[dict] = []
STATS = Counter()
...
def _probe(obs, policy_model):
    ...
    qs = {k: q_scores(MODELS[k], sf, orows, cid, otp) for k in ("Q0", "S3")}
...
def main() -> None:
    ...
    MODELS["S3"] = load_q(_FROZEN / "S3.pt")
    ...
    out = _HERE / "_l1l2_{}.jsonl.gz".format(args.tag)
