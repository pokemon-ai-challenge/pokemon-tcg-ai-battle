"""Phase16: Q0-expanded vs DeltaQ_D1 の独立長期評価(L1 / L2)。EXPLORATORY。
...
def _probe(obs, policy_model):
    ...
    before, afters, dl = _candidate_deltas(obs, me, cand, dseed)
    if dl is None:
        return None
    # §12 delta replay sanity: 同じ seed でもう一度作って一致するか
    _b2, _a2, dl2 = _candidate_deltas(obs, me, cand, dseed)
    ...
    qs = {"Q0": q_scores(MODELS["Q0"], sf, orows, cid, otp),
          "S3": dq_scores(MODELS["DQ"], sf, orows, cid, otp, dl)}
...
def main() -> None:
    ...
    MODELS["DQ"] = load_delta_q(_FROZEN / "DeltaQ_D1.pt")
    ...
    out = _HERE / "_p16_{}.jsonl.gz".format(args.tag)
