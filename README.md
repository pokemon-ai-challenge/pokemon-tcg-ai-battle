...
このコンペには、**提出期限のあとに約2週間の「最終評価期間」**があります。期限後もエージェント同士の対戦が続き、レーティングが落ち着いたところで最終順位が確定する、という公式のルールです。
...
31位は、この評価期間に入る前の数字です。提出した直後は対戦数が少ないぶんスコアが大きく振れ、実力より高く出ることがあります。

実際、31位を出した提出を、内容を一切変えないまま17日間追いかけると、スコアはこう動きました。
...
1,100点台が出たのは提出した日だけで、17日間590回の記録のうち9%でした。それ以外の期間はずっと910〜920点です。つまり**最終スコアの920.4は、このエージェントが安定して出していた実力どおりの数字**ということになります。
...
**5. オーロンゲ環境で頭打ち**
フーディンに強いオーロンゲデッキが流行し始めました。AI の判断ロジックや先読みを改善してこの対面を取りに行こうとしましたが、勝率は上がりませんでした。**デッキ相性の差はプレイングでは埋めきれない**、というのがこの段階での結論です。

**6. オーガポンデッキ ＋ 模倣学習**
オーロンゲに強いオーガポンデッキへ乗り換えました。AI 側は一切変えず、デッキだけを差し替えて約1万試合を回し、勝率が 70.1% → 76.2% に上がることを確かめてから採用しました。
...
**相手のデッキ予測を AI に渡す。** 見えているカードから相手のデッキタイプを推定するモデルを作り、推定の精度自体は出ました。しかしその予測を判断材料として渡しても、勝率は改善しませんでした。盤面の情報にすでに相手の情報が反映されており、冗長だったと考えています。
...
4人チームです。ブランチとコミットから、各メンバーが主に取り組んでいた領域を整理しました。最終提出に反映されていない試行も含みます。

**[@Showgo1130](https://github.com/Showgo1130) — 開発を支える道具と、見えない情報の推定**

対戦を1手ずつ見返せるバトルビュアーと、配布された PDF から紙のデッキを印刷するツール。相手の手札や山札に何が残っているかを推定する仕組みと、それを使った先読み。相手が何のデッキを使っているかの判別（条件分岐版と機械学習版の両方）。盤面の有利不利を点数で見積もるモデル。これらを1本の意思決定の流れにつなぐ統合作業。

**[@tsuoimorioka](https://github.com/tsuoimorioka) — 探索アルゴリズムと強化学習**

このターンで勝ち切れる手順を見つける探索。ランダムな試行をくり返して手の良し悪しを評価するモンテカルロ探索。1手にかける時間をその場の状況に応じて配分する制御。自己対戦をくり返して強くする強化学習（PPO）と、それを複数マシンに分散させる仕組み。デッキの実装。

**[@koshincarrier-pixel](https://github.com/koshincarrier-pixel) — ルールベース AI の本体**

条件分岐で行動を決めるエージェントの中核部分。デッキごとの動かし方を記述する仕組み。エネルギーをどのポケモンに付けるか、いつ「にげる」かといった個別の判断ロジック。学習モデルの再学習。相手のデッキに応じて処理を切り替える仕組み。

**[@inada107](https://github.com/inada107) — デッキの戦い方と提出**

デッキごとの戦術の実装。カードを連鎖させるコンボの処理。勝ち切る手順の探索。手を選ぶ学習モデル。相手デッキの予測。Kaggle への提出まわり。

> この内容は暫定です。各メンバーの作業ブランチには master に統合していないものがあり、統合後に見直す予定です。
