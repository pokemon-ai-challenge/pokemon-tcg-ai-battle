...
31位は、この評価期間に入る前の数字です。提出した直後は対戦数が少ないぶんスコアが大きく振れ、実力より高く出ることがあります。実際、31位を出した提出を、内容を一切変えないまま17日間追いかけると、スコアはこう動きました。
...
| メンバー | 役割 | 主に担当したこと |
|---|---|---|
| [@Showgo1130](https://github.com/Showgo1130) | **開発・分析基盤とデッキ探索** | 対戦を1手ずつ見返すバトルビュアー<br>配布 PDF から紙のデッキを刷るツール<br>Kaggle のリプレイを集めて分析する仕組み（学習データの生成もここ）<br>環境にどのデッキが何割いるかの調査<br>デッキ候補を作って多段階で絞り込む仕組み<br>改善が本物かを判定する A/B 評価と、必要な試合数の見積もり<br>提出ごとのスコアを記録し続け、どれくらい振れるかを測る仕組み<br>相手の手札・山札の推定と、それを使った先読み<br>相手が何のデッキかの判別（条件分岐版と機械学習版）<br>盤面の有利不利を点数で見積もるモデル<br>提出コード側の学習モデルと、各部品を1本の意思決定につなぐ統合 |
...
| [@koshincarrier-pixel](https://github.com/koshincarrier-pixel) | **学習モデルとルールベース AI** | 条件分岐で行動を決めるエージェントの中核部分<br>エネルギーを誰に付けるか・いつ「にげる」かの判断<br>上位プレイヤーの手を学習するモデルの学習と作り直し<br>盤面をモデルに渡すときの表し方（手札の中身・先攻後攻など）<br>学習データを増やすとどこまで伸びるかの測定<br>間違えた手の内訳を調べる仕組み<br>自己対戦による強化学習と、相手ごとの効果測定<br>相手のデッキに応じた処理の切り替え<br>Kaggle 上で学習を回す仕組み |
