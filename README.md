...
この大会のスコアは、対戦が続くかぎり更新されるレーティングです。提出期限のあとも約2週間、エージェント同士の対戦が続き、そこで最終順位が確定します。31位はその評価期間に入る前、提出から間もない時点の順位です。同じ提出のスコアを590回記録したところ、1,100点台は1件のみで、残りは900点前後に収まりました（最終スコア 920.4）。

→ [大会結果と順位分析](docs/results.md)

開発したのは AI 本体、対戦記録から学習する仕組み、AI の判断を1手ずつ見返すビュアー、手元で大量に対戦させる評価環境の4つです。
...
一般的な機械学習コンペとは、難しさの種類が違いました。主に次の4点です。
...
**フィードバックが遅い。** 提出は1日5回までで、提出してもスコアが落ち着くまでに時間がかかります。手元の改善が順位に効いたかを、1回の提出結果からは判断できません。そこで提出ごとのスコアを記録し続け、振れ幅を実測しました（追跡した提出では中央値で173点）。

**環境が動く。** 他チームが使うデッキの分布は日々変わります。自分たちが何も変えていなくても、流行のデッキが変われば勝率が変わります。終盤にデッキを変更した理由も、この環境の変化です。
...
デッキとアルゴリズムを交互に見直しながら、次の順で構成を変えています。
...
**3. フーディンデッキ ＋ 模倣学習** — ルールベースは、記述した範囲でしか判断できません。上位プレイヤーのリプレイから「その盤面で実際に選ばれた手」を教師として学習させれば、自分たちが言語化できていない判断も獲得できるのではないか、という仮説で模倣学習に移行しました。
...
この一連で手元の評価に明確な差が出たのは、デッキの変更でした。AI 側の改良による差は、ほとんどが試合ごとの運のばらつきに埋もれています。
...
いずれも手元の評価で改善を検出できず、採用を見送っています。

## チームでの進め方

研究室で「ハッカソンに出てみたい」「技術力を上げたい」という話が出ていたところに、このコンペを見つけたのが参加のきっかけです。

**紙で回してデッキを選んだ。** どのデッキが強いかを把握するため、配布されたカード PDF から実物を印刷するツールを作り、紙のポケモンカードでプレイして確かめました。

**見えないものを見えるようにした。** Kaggle 上では対戦画面しか確認できず、デッキが意図どおり動いているか、相手デッキの推定が盤面ごとに機能しているかを検証できません。そこで、リプレイを1手ずつ再生できるバトルビュアーを実装しました。
