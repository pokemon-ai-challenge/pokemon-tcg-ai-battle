...
| メンバー | 役割 | 主に担当したこと |
|---|---|---|
| [@Showgo1130](https://github.com/Showgo1130) | **開発・分析基盤とデッキ探索** | 対戦を1手ずつ見返すバトルビュアー<br>配布 PDF から紙のデッキを刷るツール<br>Kaggle のリプレイを集めて分析する仕組み（学習データの生成もここ）<br>環境にどのデッキが何割いるかの調査<br>デッキ候補を作って多段階で絞り込む仕組み<br>改善が本物かを判定する A/B 評価と、必要な試合数の見積もり<br>相手の手札・山札の推定と、それを使った先読み<br>相手が何のデッキかの判別（条件分岐版と機械学習版）<br>盤面の有利不利を点数で見積もるモデル<br>各部品を1本の意思決定につなぐ統合 |
| [@tsuoimorioka](https://github.com/tsuoimorioka) | **探索と強化学習** | このターンで勝ち切れる手順を見つける探索<br>試行をくり返して手を評価するモンテカルロ探索<br>1手にかける時間の配分制御<br>強化学習（PPO）と Transformer を使った手の選び方<br>学習を複数マシンで同時に回す仕組み<br>特定の相手に特化した戦略の組み立てと、その効果検証<br>練習相手になるルールベースのエージェント<br>デッキの実装 |
| [@koshincarrier-pixel](https://github.com/koshincarrier-pixel) | **学習モデルとルールベース AI** | 条件分岐で行動を決めるエージェントの中核部分<br>エネルギーを誰に付けるか・いつ「にげる」かの判断<br>上位プレイヤーの手を学習するモデルの学習と作り直し<br>盤面をモデルに渡すときの表し方（手札の中身・先攻後攻など）<br>学習データを増やすとどこまで伸びるかの測定<br>間違えた手の内訳を調べる仕組み<br>相手のデッキに応じた処理の切り替え<br>Kaggle 上で学習を回す仕組み |
| [@inada107](https://github.com/inada107) | **デッキ戦術と提出** | デッキごとの戦術の実装<br>カードを連鎖させるコンボの処理<br>勝ち切る手順の探索<br>手を選ぶ学習モデル<br>相手デッキの予測<br>Kaggle への提出まわり |

> この表は暫定です。ブランチとコミットから機械的に整理したものなので、実態とずれている可能性があります。また master に統合していない作業ブランチがあり、統合後に見直す予定です。
