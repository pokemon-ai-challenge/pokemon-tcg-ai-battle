...
- ランダムな試行をくり返して手の良し悪しを評価するモンテカルロ探索。「相手にダメージを蓄積させる」といったポケモンの特性を、自分から使いに行く判断も組み込んでいる
- 勝ち切れる手順を探す探索の最初の実装。あと1体倒せば勝ちという場面から作り始め、2体必要な場面まで広げた。相手のベンチのポケモンを引きずり出してから攻撃する、という手順を優先して調べる改良も入れている
- 1手にかける時間を状況に応じて配分する仕組み。制限時間を使い切りつつ超過しないよう調整する
- 自己対戦で強くしていく学習器（PPO）。盤面の良し悪しを別のモデルに評価させる構成と、させない構成の両方を作って比べた。Transformer を使った手の選び方も試している
- 報酬の設計。勝ち負けだけを手がかりにすると、1試合の最後にしか学習の材料が得られず伸びにくい。そこで「あと何体倒せば勝てるか」という途中経過も報酬に反映させ、学習を安定させた
- 弱い相手と当たっただけで手が良かったように見えてしまう問題への対処。手の評価を相手ごとにならし、相手の強さによる差を取り除いた
- 学習を複数マシン・Kaggle 上で同時に回す仕組み。学習が頭打ちになる原因を、1種類の相手に対して回せる試合数が足りていない点まで絞り込んだ
- オーガポン用に、決まった手順を通す打ち方をあらかじめ用意して選ばせる仕組み。使った場合と使わなかった場合を比べたが効果を確認できず、開発を打ち切る判断まで行った
...
- 条件分岐で行動を決めるエージェントの中核。「攻撃する」「場を整える」「カードを引く」といった行動の種類ごとに点数を付け、その合計で何をするか決める仕組み
- エネルギーを誰に付けるかの判断。付け先を間違えると攻撃が成立しないため、条件を何度も作り直している
- 「にげる」の判断。無駄なにげるが多発していたため、ロジックを設計し直した
- 攻撃の前に場を整えるか、すぐ攻撃するかの判断と、グッズ・サポート・スタジアムといったカードの種類ごとに、使ってよい条件の判定
- ドロー手段の確保。ベンチにドロー役の枠を予約し、毎ターン発動させる
- カードのテキストから、状況で変わる技のダメージを見積もる処理
- モデルに盤面を渡すときの表し方の拡張。手札に何があるか、先攻か後攻か、相手の見えているカード、決着までの残りターンの見込み、山札の減る速さ、といった情報を段階的に足していった
...
- 勝ち切る手順を見つける探索の本体。「このターンで倒し切れる／倒し切れない／わからない」の3通りで判定し、確率が絡む効果にも対応させた。最終エージェントの1段目にあたる部分
